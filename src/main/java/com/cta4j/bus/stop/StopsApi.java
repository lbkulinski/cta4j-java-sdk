...
public interface StopsApi {
    ...
    /// @return an [Optional] containing the [Stop] if found, or an empty [Optional] if no stop is found for the given
    /// ID
    /// @throws NullPointerException if `stopId` is `null`
    /// @throws Cta4jBusException if multiple stops are found for the given ID, or if the API returns an error response
    ...
    /// @return a [List] of [Stop]s corresponding to the provided route ID and direction, or an empty [List] if no
    /// stops are found
