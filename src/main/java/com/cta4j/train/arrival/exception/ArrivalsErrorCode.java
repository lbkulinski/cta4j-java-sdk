...
public enum ArrivalsErrorCode {
    ...
    /// Indicates that a noninteger value was specified for the maximum number of results.
    INVALID_MAX(110),
    ...
    /// Returns the `ArrivalsErrorCode` corresponding to the given integer code.
    ///
    /// @param code the integer code to look up
    /// @return the corresponding `ArrivalsErrorCode`, or `null` if the code does not match any defined error code
    public static @Nullable ArrivalsErrorCode fromCode(int code) {
        return switch (code) {
            case 0 -> OK;
