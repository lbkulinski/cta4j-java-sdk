...
///
/// @param score the numerical score used to rank this severity, based on the alert's impact on overall service,
...
public record Severity(
    ...
    ///
    /// @param score the numerical score used to rank the severity, based on the alert's impact on overall service,
    ///              between 0 and 99 (inclusive)
    ...
    /// @throws NullPointerException if `color` or `css` is `null`
    /// @throws IllegalArgumentException if `score` is not between 0 and 99 (inclusive)
    public Severity {
